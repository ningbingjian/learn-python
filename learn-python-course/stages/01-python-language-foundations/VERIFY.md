...
PR 审查后进一步收紧验证器：枚举全部编号目录并精确比较七个 Module 和 21 个 Unit 的名称；从状态字段读取完整值，不接受 NOT BUILT、UNBUILT 或正文中提及 BUILT。另有 **8 项作者工具回归测试**，验证额外目录、改名、缺失 README、错误或缺失状态会被拒绝；不计入上述 104 项课程行为测试。作者工具测试的临时夹具不属于教学 Unit。

循环导入断言只匹配缺失的 success_message 名字与参与文件，兼容后续出现普通模块名或 partially initialized module 的诊断。修复后全套验证、工具回归和格式检查重新通过。
